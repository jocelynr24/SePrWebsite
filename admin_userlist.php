			...
			<table width=930 height=200 border=0><tr><td align=left>
				...
				<center>
					...
					if (isset($_SESSION["logged"]) && ($_SESSION["role"] == 1)){
						...
						$request = $PDO->query("SELECT id, login, email, role, firstname, lastname FROM users ORDER BY ID");
						echo '
						...
						while ($row = $request->fetch(PDO::FETCH_ASSOC)){
							...
							echo '<tr><td class="table_td">'.$row['id'].' - <a href="admin_userdetail.php?id='.$row['id'].'">edit</a></td><td class="table_td">'.$role.'</td><td class="table_td">'.$row['login'].'</td><td class="table_td">'.$row['firstname'].'</td><td class="table_td">'.$row['lastname'].'</td><td class="table_td">'.$row['email'].'</td>';
						}
						$request->closeCursor();
						echo '</table>';
						} else {
